...
data_base_region="DE"
data_file="RegularParameters_v1"
hourly_data_file = "Timeseries"
threads=6
...
if termination_status(model) == MOI.INFEASIBLE_OR_UNBOUNDED
    ...
    open("/cluster/home/danare/git/GENeSYS_MOD-main/dev_jl/results/iis.txt", "w") do file
...
elseif termination_status(model) == MOI.OPTIMAL || termination_status(model) == MOI.ALMOST_INFEASIBLE
    ...
    file = open(normpath(joinpath(dirname(@__FILE__),"..","..", "dev_jl", "results", "skip12h2$dt.txt")), "w")
    ...
    file = open(normpath(joinpath(dirname(@__FILE__),"..","..", "dev_jl", "results", "runtime$dt.txt")), "w")
    # Write variable names and values to the file
    println(file, "$(length(all_variables(model)))")
    println(file, "(Total : ",Dates.now()-start,"\n)")
