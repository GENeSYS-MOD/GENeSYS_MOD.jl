...
solver_names = ["simplex", "pdlp"]
building_time = Dict("simplex" => [], "pdlp" => [])
solving_time = Dict("simplex" => [], "pdlp" => [])
objective_list = Dict("simplex" => [], "pdlp" => [])
n_var = Dict("simplex" => [], "pdlp" => [])
n_constr = Dict("simplex" => [], "pdlp" => [])
...
for solver_name in solver_names
    ...
    if string(solver) == "HiGHS.Optimizer"
    ...
    push!(building_time[solver_name], build_time)
    push!(solving_time[solver_name], solve_time)
    push!(objective_list[solver_name], obj_value)
    push!(n_var[solver_name], n_vars)
    push!(n_constr[solver_name], n_constraints)
...
for (b, s, o, v, c) in zip(building_time["simplex"], solving_time["simplex"], objective_list["simplex"], n_var["simplex"], n_constr["simplex"])
    println(io, "simplex: Build Time: $b, Solve Time: $s, Obj: $o, Vars: $v, Constr: $c")
end

for (b, s, o, v, c) in zip(building_time["pdlp"], solving_time["pdlp"], objective_list["pdlp"], n_var["pdlp"], n_constr["pdlp"])
    println(io, "pdlp: Build Time: $b, Solve Time: $s, Obj: $o, Vars: $v, Constr: $c")
end
close(io)
