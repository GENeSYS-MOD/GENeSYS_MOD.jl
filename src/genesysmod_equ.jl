      ...
      for f ∈ 𝓕
        if Params.TradeRoute[r,rr,f,𝓨[i]] > 0
          ...
          if f == "Power" && i > 1 && Params.GrowthRateTradeCapacity[r,rr,f,𝓨[i]] > 0 
            @constraint(model, (Params.GrowthRateTradeCapacity[r,rr,f,𝓨[i]]*YearlyDifferenceMultiplier(𝓨[i],Sets))*model[:TotalTradeCapacity][𝓨[i],f,r,rr] >= model[:NewTradeCapacity][𝓨[i],f,r,rr], 
 ...
 ############### Total Discounted Costs #############
  ...
  for r ∈ 𝓡 for s ∈ 𝓢 for i ∈ eachindex(𝓨)
    if i == 1
      ...
      @constraint(model, 
      model[:StorageLevelYearStart][s,𝓨[i-1],r] + sum((sum(model[:RateOfActivity][𝓨[i],l,t,m,r] * Params.TechnologyToStorage[t,s,m,𝓨[i]] for m ∈ 𝓜 for t ∈ Subsets.StorageDummies if Params.TechnologyToStorage[t,s,m,𝓨[i]]>0)
          - sum( model[:RateOfActivity][𝓨[i],l,t,m,r] / Params.TechnologyFromStorage[t,s,m,𝓨[i]] for m ∈ 𝓜 for t ∈ Subsets.StorageDummies if Params.TechnologyFromStorage[t,s,m,𝓨[i]]>0)) * Params.YearSplit[l,𝓨[i]] for l ∈ 𝓛)
  ...
  if Switch.switch_peaking_capacity == 1
    ...
    for y ∈ 𝓨 for r ∈ 𝓡
      ...
      if y >Switch.set_peaking_startyear
        @constraint(model,
        model[:PeakingCapacity][y,r] + (Switch.switch_peaking_with_trade == 1 ? sum(model[:TotalTradeCapacity][y,"Power",r,rr] for rr ∈ 𝓡) : 0)
