...
#=     for f ∈ 𝓕
  ...
  start=Dates.now()
  for y ∈ 𝓨 for t ∈ 𝓣 for r ∈ 𝓡
    if !isempty(Maps.Tech_MO[t]) && (sum(Params.VariableCost[r,t,m,y] for m ∈ Maps.Tech_MO[t]) > 0) && (CanBuildTechnology[y,t,r] > 0)
 ...
 ############### Total Discounted Costs #############
  ...
  for i ∈ eachindex(𝓨) for f ∈ 𝓕 for r ∈ 𝓡
    @constraint(model,
    sum(Vars.ProductionByTechnologyAnnual[𝓨[i],t,f,r] for t ∈ Params.TagTechnologyToSubsets["Renewables"] ) == Vars.TotalREProductionAnnual[𝓨[i],r,f],base_name="RE1_ComputeTotalAnnualREProduction|$(𝓨[i])|$(r)|$(f)")
    ...
    if Switch.switch_dispatch == 0
      if 𝓨[i]> Switch.StartYear && Params.SpecifiedAnnualDemand[r,f,𝓨[i]]>0 && f != "H2"
        @constraint(model,
  ...
  if Switch.switch_peaking_capacity == 1
    ...
    for y ∈ 𝓨 for r ∈ 𝓡
      ...
      if y > Switch.set_peaking_startyear
        @constraint(model,
            Vars.PeakingCapacity[y,r] + 
            (Switch.switch_peaking_with_trade == 1 ? sum(Vars.TotalTradeCapacity[y, "Power", rr, r] for rr ∈ 𝓡; init=0) : 0) +
            (Switch.switch_peaking_with_storages == 1 ? 
                sum(Vars.TotalCapacityAnnual[y, t, r] for t ∈ setdiff(𝓣, Params.TagTechnologyToSubsets["StorageDummies"])
                    if (sum(Params.OutputActivityRatio[r, t, "Power", m, y] for m ∈ Maps.Tech_MO[t]; init=0) != 0); init=0) : 0)
