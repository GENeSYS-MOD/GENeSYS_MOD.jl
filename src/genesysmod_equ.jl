...
function genesysmod_equ(model,Sets,Subsets,Params, Vars,Emp_Sets,Settings,Switch, Maps)
  ...
  + sum(Vars.BaseYearOvershoot[r,t,"Power",y]*999 for y ∈ 𝓨 for r ∈ 𝓡 for t ∈ 𝓣))
  ...
  for y ∈ 𝓨 for t ∈ 𝓣 for  r ∈ 𝓡
    ...
    if cond
      @constraint(model, Vars.AccumulatedNewCapacity[y,t,r] == sum(Vars.NewCapacity[yy,t,r] for yy ∈ 𝓨 if (y - yy < Params.OperationalLife[r,t]) && (y-yy>= 0)), base_name="CA1_TotalNewCapacity_$(y)_$(t)_$(r)")
    else
  ...
  if Switch.switch_intertemporal == 1
    for r ∈ 𝓡 for l ∈ 𝓛 for t ∈ 𝓣 for y ∈ 𝓨
      ...
      if (sum(Params.CapacityFactor[r,t,l,yy] for yy ∈ 𝓨 if y-yy < Params.OperationalLife[r,t] && y-yy >= 0) > 0 || Params.CapacityFactor[r,t,l,Switch.StartYear] > 0) && Params.TotalTechnologyModelPeriodActivityUpperLimit[r,t] > 0 && Params.AvailabilityFactor[r,t,y] > 0 && Params.TotalAnnualMaxCapacity[r,t,y] > 0
        @constraint(model,
        Vars.TotalActivityPerYear[r,l,t,y] == sum(Vars.NewCapacity[yy,t,r] * Params.CapacityFactor[r,t,l,yy] * Params.CapacityToActivityUnit[r,t] for yy ∈ 𝓨 if y-yy < Params.OperationalLife[r,t] && y-yy >= 0)+(Params.ResidualCapacity[r,t,y]*Params.CapacityFactor[r,t,l,Switch.StartYear] * Params.CapacityToActivityUnit[r,t]),
    ...
    for y ∈ 𝓨 for t ∈ 𝓣 for  r ∈ 𝓡 for l ∈ 𝓛
      if (Params.CapacityFactor[r,t,l,y] > 0) &&
        ...
        (Params.TotalAnnualMaxCapacity[r,t,y] > 0) &&
        (Params.TotalTechnologyModelPeriodActivityUpperLimit[r,t] > 0)
          @constraint(model, sum(Vars.RateOfActivity[y,l,t,m,r] for m ∈ Maps.Tech_MO[t]) == Vars.TotalCapacityAnnual[y,t,r] * Params.CapacityFactor[r,t,l,y] * Params.CapacityToActivityUnit[r,t] * Params.AvailabilityFactor[r,t,y] - Vars.DispatchDummy[r,l,t,y] * Params.TagDispatchableTechnology[t] - Vars.CurtailedCapacity[r,l,t,y] * Params.CapacityToActivityUnit[r,t],
          base_name="CA3b_RateOfTotalActivity_$(r)_$(l)_$(t)_$(y)")
      end
      @constraint(model, Vars.TotalCapacityAnnual[y,t,r] >= Vars.CurtailedCapacity[r,l,t,y], base_name="CA3c_CurtailedCapacity_$(r)_$(t)_$(y)")
    end end end end
  end
  ...
  for y ∈ 𝓨 for t ∈ 𝓣 for  r ∈ 𝓡
    if (Params.AvailabilityFactor[r,t,y] < 1) &&
      ...
      ((JuMP.is_fixed(Vars.TotalCapacityAnnual[y,t,r])) && (JuMP.fix_value(Vars.TotalCapacityAnnual[y,t,r]) > 0)))
      @constraint(model, sum(sum(Vars.RateOfActivity[y,l,t,m,r]  for m ∈ Maps.Tech_MO[t]) * Params.YearSplit[l,y] for l ∈ 𝓛) <= sum(Vars.TotalCapacityAnnual[y,t,r]*Params.CapacityFactor[r,t,l,y]*Params.YearSplit[l,y]*Params.AvailabilityFactor[r,t,y]*Params.CapacityToActivityUnit[r,t] for l ∈ 𝓛), base_name="CA5_CapacityAdequacy_$(y)_$(t)_$(r)")
    end
  end end end
  ...
  for y ∈ 𝓨 for f ∈ 𝓕 for r ∈ 𝓡
    ...
    if sum(Params.TradeRoute[y,f,r,rr] for rr ∈ 𝓡) == 0
      ...
      for l ∈ 𝓛
        @constraint(model, sum(Vars.Export[y,l,f,r,rr]*(1+Params.TradeLossBetweenRegions[y,f,r,rr]) - Vars.Import[y,l,f,r,rr] for rr ∈ 𝓡 if Params.TradeRoute[y,f,r,rr] > 0) == Vars.NetTrade[y,l,f,r], 
  ...
  start=Dates.now()
  for y ∈ 𝓨 for f ∈ 𝓕 for r ∈ 𝓡
    @constraint(model, Vars.CurtailedEnergyAnnual[y,f,r] == sum(Vars.CurtailedCapacity[r,l,t,y] * Params.OutputActivityRatio[r,t,f,m,y] * Params.YearSplit[l,y] * Params.CapacityToActivityUnit[r,t] for l ∈ 𝓛 for (t,m) ∈ LoopSetOutput[(r,f,y)]), 
  ...
  for i ∈ eachindex(𝓨) for r ∈ 𝓡 for rr ∈ 𝓡
    if Params.TradeRoute[𝓨[i],"Power",rr,r] > 0
      for l ∈ 𝓛
        @constraint(model, (Vars.Import[𝓨[i],l,"Power",r,rr]) <= Vars.TotalTradeCapacity[𝓨[i],"Power",rr,r]*Params.YearSplit[l,𝓨[i]]*31.536 , base_name="TrC1_TradeCapacityPowerLinesImport_$(𝓨[i])_$(l)_Power_$(r)_$(rr)")
      end
    end
    if Params.TradeRoute[𝓨[i],"Power",r,rr] > 0
      for l ∈ 𝓛
        @constraint(model, (Vars.Export[𝓨[i],l,"Power",r,rr]) <= Vars.TotalTradeCapacity[𝓨[i],"Power",r,rr]*Params.YearSplit[l,𝓨[i]]*31.536 , base_name="TrC1_TradeCapacityPowerLinesExport_$(𝓨[i])_$(l)_Power_$(r)_$(rr)")
      end
      @constraint(model, Vars.NewTradeCapacity[𝓨[i],"Power",r,rr]*Params.TradeCapacityGrowthCosts["Power",r,rr]*Params.TradeRoute[𝓨[i],"Power",r,rr] == Vars.NewTradeCapacityCosts[𝓨[i],"Power",r,rr], base_name="TrC4_NewTradeCapacityCosts_$(𝓨[i])_Power_$(r)_$(rr)")
      ...
      for f ∈ 𝓕
        if Params.TradeRoute[𝓨[i],f,r,rr] > 0
          if 𝓨[i] == Switch.StartYear
            @constraint(model, Vars.TotalTradeCapacity[𝓨[i],f,r,rr] == Params.TradeCapacity[𝓨[i],f,r,rr], base_name="TrC2a_TotalTradeCapacityStartYear_$(𝓨[i])_$(f)_$(r)_$(rr)")
          elseif 𝓨[i] > Switch.StartYear
            @constraint(model, Vars.TotalTradeCapacity[𝓨[i],f,r,rr] == Vars.TotalTradeCapacity[𝓨[i-1],f,r,rr] + Vars.NewTradeCapacity[𝓨[i],f,r,rr] + Params.CommissionedTradeCapacity[𝓨[i],f,r,rr], 
          ...
          if i > 1 && Params.GrowthRateTradeCapacity[𝓨[i],f,r,rr] > 0 && Params.TradeRoute[𝓨[i],f,r,rr] > 0
            @constraint(model, (Params.GrowthRateTradeCapacity[𝓨[i],f,r,rr]*YearlyDifferenceMultiplier(𝓨[i],Sets))*Vars.TotalTradeCapacity[𝓨[i-1],f,r,rr] >= Vars.NewTradeCapacity[𝓨[i],f,r,rr], 
  ...
  for y ∈ 𝓨 for r ∈ 𝓡
    if sum(Params.TradeRoute[y,f,r,rr] for f ∈ 𝓕 for rr ∈ 𝓡) > 0
      @constraint(model, sum(Vars.Import[y,l,f,r,rr] * Params.TradeCosts[f,r,rr] for f ∈ 𝓕 for rr ∈ 𝓡 for l ∈ 𝓛 if Params.TradeRoute[y,f,r,rr] > 0) == Vars.AnnualTotalTradeCosts[y,r], base_name="Tc1_TradeCosts_$(y)_$(r)")
    else
  ...
  for y ∈ 𝓨 for t ∈ 𝓣 for  r ∈ 𝓡 for m ∈ Maps.Tech_MO[t]
    if CanBuildTechnology[y,t,r] > 0
      @constraint(model, sum(Vars.RateOfActivity[y,l,t,m,r]*Params.YearSplit[l,y] for l ∈ 𝓛) == Vars.TotalAnnualTechnologyActivityByMode[y,t,m,r], base_name="Acc3_AverageAnnualRateOfActivity_$(y)_$(t)_$(m)_$(r)")
    else
  ...
  start=Dates.now()
  for r ∈ 𝓡
    @constraint(model, sum(Vars.TotalDiscountedCost[y,r] for y ∈ 𝓨) == Vars.ModelPeriodCostByRegion[r], base_name="Acc4_ModelPeriodCostByRegion_$(r)")
  end
      ...
      if Switch.switch_ccs == 1
        for r ∈ 𝓡
          ...
          if sum(Params.RegionalCCSLimit[r] for r ∈ 𝓡)>0
            @constraint(model,
            sum(sum( Vars.TotalAnnualTechnologyActivityByMode[y,t,m,r]*Params.EmissionContentPerFuel[f,e]*Params.InputActivityRatio[r,t,f,m,y]*YearlyDifferenceMultiplier(y,Sets)*((Params.EmissionActivityRatio[r,t,e,m,y]>0 ? (1-Params.EmissionActivityRatio[r,t,e,m,y]) : 0)+
            (Params.EmissionActivityRatio[r,t,e,m,y] < 0 ? (-1)*Params.EmissionActivityRatio[r,t,e,m,y] : 0)) for f ∈ Maps.Tech_Fuel[t] for m ∈ Maps.Tech_MO[t] for e ∈ 𝓔) for y ∈ 𝓨 for t ∈ Subsets.CCS ) <= Params.RegionalCCSLimit[r],
  ...
  for y ∈ 𝓨 for t ∈ 𝓣 for r ∈ 𝓡
    if Settings.DepreciationMethod[r]==1 && ((y + Params.OperationalLife[r,t] - 1 > max(𝓨...)) && (Settings.TechnologyDiscountRate[r,t] > 0))
      @constraint(model, 
      Vars.SalvageValue[y,t,r] == Params.CapitalCost[r,t,y]*Vars.NewCapacity[y,t,r]*(1-(((1+Settings.TechnologyDiscountRate[r,t])^(max(𝓨...) - y + 1 ) -1)/((1+Settings.TechnologyDiscountRate[r,t])^Params.OperationalLife[r,t]-1))),
      base_name="SV1_SalvageValueAtEndOfPeriod1_$(y)_$(t)_$(r)")
    end

    if (((y + Params.OperationalLife[r,t]-1 > max(𝓨...)) && (Settings.TechnologyDiscountRate[r,t] == 0)) || (Settings.DepreciationMethod[r]==2 && (y + Params.OperationalLife[r,t]-1 > max(𝓨...))))
      @constraint(model,
      Vars.SalvageValue[y,t,r] == Params.CapitalCost[r,t,y]*Vars.NewCapacity[y,t,r]*(1-(max(𝓨...)- y+1)/Params.OperationalLife[r,t]),
      base_name="SV2_SalvageValueAtEndOfPeriod2_$(y)_$(t)_$(r)")
    end
    if y + Params.OperationalLife[r,t]-1 <= max(𝓨...)
      @constraint(model,
  ...
  for y ∈ 𝓨 for t ∈ 𝓣 for r ∈ 𝓡
    ...
    if (Params.FixedCost[r,t,y] > 0) & (CanBuildTechnology[y,t,r] > 0)
      @constraint(model, sum(Vars.NewCapacity[yy,t,r]*Params.FixedCost[r,t,yy] for yy ∈ 𝓨 if (y-yy < Params.OperationalLife[r,t]) && (y-yy >= 0)) == Vars.AnnualFixedOperatingCost[y,t,r], base_name="OC2_OperatingCostsFixedAnnual_$(y)_$(t)_$(r)")
    else
 ...
 ############### Total Discounted Costs #############
  ...
  for i ∈ eachindex(𝓨) for f ∈ 𝓕 for r ∈ 𝓡
    for t ∈ Maps.Fuel_Tech[f]
      if sum(Params.OutputActivityRatio[r,t,f,m,𝓨[i]] for m ∈ Maps.Tech_MO[t]) > 0 &&
        ...
        @constraint(model, sum(sum(Vars.RateOfActivity[𝓨[i],l,t,m,r]*Params.OutputActivityRatio[r,t,f,m,𝓨[i]] for m ∈ Maps.Tech_MO[t] if Params.OutputActivityRatio[r,t,f,m,𝓨[i]] != 0)* Params.YearSplit[l,𝓨[i]] for l ∈ 𝓛) == Vars.ProductionByTechnologyAnnual[𝓨[i],t,f,r], base_name= "RE1_FuelProductionByTechnologyAnnual_$(𝓨[i])_$(t)_$(f)_$(r)")
      else
        JuMP.fix(Vars.ProductionByTechnologyAnnual[𝓨[i],t,f,r],0;force=true)
      end

      if sum(Params.InputActivityRatio[r,t,f,m,𝓨[i]] for m ∈ Maps.Tech_MO[t]) > 0 &&
        Params.AvailabilityFactor[r,t,𝓨[i]] > 0 &&
        Params.TotalAnnualMaxCapacity[r,t,𝓨[i]] > 0 &&
        Params.TotalTechnologyModelPeriodActivityUpperLimit[r,t] > 0 &&
        (((JuMP.has_upper_bound(Vars.TotalCapacityAnnual[𝓨[i],t,r])) && (JuMP.upper_bound(Vars.TotalCapacityAnnual[𝓨[i],t,r]) > 0)) ||
        ((!JuMP.has_upper_bound(Vars.TotalCapacityAnnual[𝓨[i],t,r])) && (!JuMP.is_fixed(Vars.TotalCapacityAnnual[𝓨[i],t,r]))) ||
        ((JuMP.is_fixed(Vars.TotalCapacityAnnual[𝓨[i],t,r])) && (JuMP.fix_value(Vars.TotalCapacityAnnual[𝓨[i],t,r]) > 0)))
        @constraint(model, sum(sum(Vars.RateOfActivity[𝓨[i],l,t,m,r]*Params.InputActivityRatio[r,t,f,m,𝓨[i]] for m ∈ Maps.Tech_MO[t] if Params.InputActivityRatio[r,t,f,m,𝓨[i]] != 0)* Params.YearSplit[l,𝓨[i]] for l ∈ 𝓛) == Vars.UseByTechnologyAnnual[𝓨[i],t,f,r], base_name= "RE5_FuelUseByTechnologyAnnual_$(𝓨[i])_$(t)_$(f)_$(r)")
      else
    ...
    @constraint(model,
    sum(Vars.ProductionByTechnologyAnnual[𝓨[i],t,f,r] for t ∈ Subsets.Renewables ) == Vars.TotalREProductionAnnual[𝓨[i],r,f],base_name="RE2_TechIncluded_$(𝓨[i])_$(r)_$(f)")
  ...
  for r ∈ 𝓡 for s ∈ 𝓢 for i ∈ eachindex(𝓨)
    if i == 1
      JuMP.fix(Vars.StorageLevelYearStart[s, 𝓨[i], r], Params.StorageLevelStart[r,s];force=true)
    else
      @constraint(model, 
      Vars.StorageLevelYearStart[s,𝓨[i-1],r] + sum((sum(Vars.RateOfActivity[𝓨[i],l,t,m,r] * Params.TechnologyToStorage[𝓨[i],m,t,s] for t ∈ Subsets.StorageDummies for m ∈ Maps.Tech_MO[t] if Params.TechnologyToStorage[𝓨[i],m,t,s]>0)
          - sum( Vars.RateOfActivity[𝓨[i],l,t,m,r] / Params.TechnologyFromStorage[𝓨[i],m,t,s] for t ∈ Subsets.StorageDummies for m ∈ Maps.Tech_MO[t] if Params.TechnologyFromStorage[𝓨[i],m,t,s]>0)) * Params.YearSplit[l,𝓨[i]] for l ∈ 𝓛)
      == Vars.StorageLevelYearStart[s,𝓨[i],r],
      base_name="S1_StorageLevelYearStart_$(r)_$(s)_$(𝓨[i])")
      
      JuMP.fix(Vars.StorageLevelYearStart[s, 𝓨[i], r], 0;force=true)
    end
    
    @constraint(model,
    sum((sum(Vars.RateOfActivity[𝓨[i],l,t,m,r] * Params.TechnologyToStorage[𝓨[i],m,t,s] for t ∈ Subsets.StorageDummies  for m ∈ Maps.Tech_MO[t] if Params.TechnologyToStorage[𝓨[i],m,t,s]>0)
              - sum(Vars.RateOfActivity[𝓨[i],l,t,m,r] / Params.TechnologyFromStorage[𝓨[i],m,t,s] for t ∈ Subsets.StorageDummies for m ∈ Maps.Tech_MO[t] if Params.TechnologyFromStorage[𝓨[i],m,t,s]>0)) for l ∈ 𝓛) == 0,
              base_name="S3_StorageRefilling_$(r)_$(s)_$(𝓨[i])")
    for j ∈ eachindex(𝓛)
      @constraint(model,
      (j>1 ? Vars.StorageLevelTSStart[s,𝓨[i],𝓛[j-1],r] + 
      (sum((Params.TechnologyToStorage[𝓨[i],m,t,s]>0 ? Vars.RateOfActivity[𝓨[i],𝓛[j-1],t,m,r] * Params.TechnologyToStorage[𝓨[i],m,t,s] : 0) for t ∈ Subsets.StorageDummies for m ∈ Maps.Tech_MO[t])
        - sum((Params.TechnologyFromStorage[𝓨[i],m,t,s]>0 ? Vars.RateOfActivity[𝓨[i],𝓛[j-1],t,m,r] / Params.TechnologyFromStorage[𝓨[i],m,t,s] : 0 ) for t ∈ Subsets.StorageDummies for m ∈ Maps.Tech_MO[t])) * Params.YearSplit[𝓛[j-1],𝓨[i]] : 0)
      ...
      @constraint(model,
      sum(Vars.NewStorageCapacity[s,𝓨[i],r] + Params.ResidualStorageCapacity[r,s,𝓨[i]] for yy ∈ 𝓨 if (𝓨[i]-yy < Params.OperationalLifeStorage[r,s,yy] && 𝓨[i]-yy >= 0))
    ...
    if ((Settings.DepreciationMethod[r]==1 && (𝓨[i]+Params.OperationalLifeStorage[r,s,𝓨[i]]-1) > 𝓨[end] && Settings.GeneralDiscountRate[r]==0) || (Settings.DepreciationMethod[r]==2 && (𝓨[i]+Params.OperationalLifeStorage[r,s,𝓨[i]]-1) > 𝓨[end] && Settings.GeneralDiscountRate[r]==0))
      @constraint(model,
      Vars.CapitalInvestmentStorage[s,𝓨[i],r]*(1- 𝓨[end] - 𝓨[i]+1)/Params.OperationalLifeStorage[r,s,𝓨[i]] == Vars.SalvageValueStorage[s,𝓨[i],r],
    ...
    if (Settings.DepreciationMethod[r]==1 && ((𝓨[i]+Params.OperationalLifeStorage[r,s,𝓨[i]]-1) > 𝓨[end] && Settings.GeneralDiscountRate[r]>0))
      @constraint(model,
      Vars.CapitalInvestmentStorage[s,𝓨[i],r]*(1-((1+Settings.GeneralDiscountRate[r])^(𝓨[end] - 𝓨[i]+1)-1)/((1+Settings.GeneralDiscountRate[r])^Params.OperationalLifeStorage[r,s,𝓨[i]]-1)) == Vars.SalvageValueStorage[s,𝓨[i],r],
  ...
  for s ∈ 𝓢 for i ∈ eachindex(𝓨)
    for r ∈ 𝓡 
      if Params.MinStorageCharge[r,s,𝓨[i]] > 0
        for j ∈ eachindex(𝓛)
          @constraint(model, 
          Params.MinStorageCharge[r,s,𝓨[i]]*sum(Vars.NewStorageCapacity[s,𝓨[i],r] + Params.ResidualStorageCapacity[r,s,𝓨[i]] for yy ∈ 𝓨 if (𝓨[i]-yy < Params.OperationalLifeStorage[r,s,yy] && 𝓨[i]-yy >= 0))
          <= Vars.StorageLevelTSStart[s,𝓨[i],𝓛[j],r],
    ...
    for t ∈ Subsets.StorageDummies for m ∈ Maps.Tech_MO[t]
      if Params.TechnologyFromStorage[𝓨[i],m,t,s]>0
        for r ∈ 𝓡 for j ∈ eachindex(𝓛)
          @constraint(model,
          Vars.RateOfActivity[𝓨[i],𝓛[j],t,m,r]/Params.TechnologyFromStorage[𝓨[i],m,t,s]*Params.YearSplit[𝓛[j],𝓨[i]] <= Vars.StorageLevelTSStart[s,𝓨[i],𝓛[j],r],
        ...
        for i ∈ eachindex(𝓛)
          if i>1
            ...
            if Params.TagDispatchableTechnology[t]==1 && Params.RampingUpFactor[r,t,y] != 0 && Params.AvailabilityFactor[r,t,y] > 0 && Params.TotalAnnualMaxCapacity[r,t,y] > 0 && Params.TotalTechnologyModelPeriodActivityUpperLimit[r,t] > 0
              @constraint(model,
              Vars.ProductionUpChangeInTimeslice[y,𝓛[i],f,t,r] <= Vars.TotalCapacityAnnual[y,t,r]*Params.AvailabilityFactor[r,t,y]*Params.CapacityToActivityUnit[r,t]*Params.RampingUpFactor[r,t,y]*Params.YearSplit[𝓛[i],y],
            ...
            if Params.TagDispatchableTechnology[t]==1 && Params.RampingDownFactor[r,t,y] != 0 && Params.AvailabilityFactor[r,t,y] > 0 && Params.TotalAnnualMaxCapacity[r,t,y] > 0 && Params.TotalTechnologyModelPeriodActivityUpperLimit[r,t] > 0
              @constraint(model,
              Vars.ProductionDownChangeInTimeslice[y,𝓛[i],f,t,r] <= Vars.TotalCapacityAnnual[y,t,r]*Params.AvailabilityFactor[r,t,y]*Params.CapacityToActivityUnit[r,t]*Params.RampingDownFactor[r,t,y]*Params.YearSplit[𝓛[i],y],
          ...
          if Params.MinActiveProductionPerTimeslice[y,𝓛[i],f,t,r] > 0
            ...
            Vars.TotalCapacityAnnual[y,t,r]*Params.AvailabilityFactor[r,t,y]*Params.CapacityToActivityUnit[r,t]*Params.MinActiveProductionPerTimeslice[y,𝓛[i],f,t,r],
  ...
  for y ∈ 𝓨 for l ∈ 𝓛 for f ∈ 𝓕 for r ∈ 𝓡
    @constraint(model,
    sum(Vars.CurtailedCapacity[r,l,t,y] * Params.OutputActivityRatio[r,t,f,m,y] * Params.YearSplit[l,y] * Params.CapacityToActivityUnit[r,t] for (t,m) ∈ LoopSetOutput[(r,f,y)]) == Vars.CurtailedEnergy[y,l,f,r],
      ...
      if Switch.switch_peaking_minrun == 1
        for t ∈ 𝓣
          if (Params.TagTechnologyToSector[t,"Power"]==1 && Params.AvailabilityFactor[r,t,y]<=1 && 
            ...
            sum(Vars.TotalCapacityAnnual[y,t,r]*Params.CapacityFactor[r,t,l,y]*Params.YearSplit[l,y]*Params.AvailabilityFactor[r,t,y]*Params.CapacityToActivityUnit[r,t] for l ∈ 𝓛 )*MinRunShare,
