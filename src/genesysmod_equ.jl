    ...
    if Switch.switch_ccs == 1
      for r ∈ 𝓡
        ...
        if sum(Params.RegionalCCSLimit[r] for r ∈ 𝓡)>0
          ...
          (Params.EmissionActivityRatio[r,t,m,e,y] < 0 ? (-1)*Params.EmissionActivityRatio[r,t,m,e,y] : 0)) for f ∈ Maps.Tech_Fuel[t] for m ∈ Maps.Tech_MO[t] for e ∈ 𝓔) for y ∈ 𝓨 for t ∈ intersect(𝓣,intersect(𝓣,Params.TagTechnologyToSubsets["CCS"]))) <= Params.RegionalCCSLimit[r],
