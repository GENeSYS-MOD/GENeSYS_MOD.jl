...
#=     for r ∈ Sets.Region_full
    ...
    for r ∈ Sets.Region_full for t ∈ Params.Tags.TagTechnologyToSubsets["Transport"]
    ...
    #
    subs = vcat(Params.Tags.TagTechnologyToSubsets["Solar"], Params.Tags.TagTechnologyToSubsets["Wind"], ["RES_Hydro_Small"])
    Params.Tags.TagDispatchableTechnology[subs] = zeros(length(intersect(Sets.Technology,subs)))
    Params.CurtailmentCostFactor == 0.1

    for r ∈ Sets.Region_full for t ∈ Params.Tags.TagTechnologyToSubsets["Solar"]
    ...
    if Switch.switch_ccs == 1
        ...
        for y ∈ Sets.Year for r ∈ Sets.Region_full for t ∈ Params.Tags.TagTechnologyToSubsets["CCS"]
